...
using Content.Shared._ClawCommand.HereticAdapters;
    ...
    private void OnLivingHeartActivate(EventHereticLivingHeartActivate args)
    {
        ...
        else
        {
            ...
            if (isOnStation && distance <= 50f)
                _aud.PlayGlobal(new SoundPathSpecifier("/Audio/_ClawCommand/Heretic/heartbeat.ogg"), target.Value, AudioParams.Default.WithVolume(-distance * 0.5f)); // steep dropoff so it's more impactful when they're close
        }

        Popup.PopupEntity(loc, uid, uid, PopupType.Medium);
        _aud.PlayGlobal(new SoundPathSpecifier("/Audio/_ClawCommand/Heretic/heartbeat.ogg"), uid, AudioParams.Default.WithVolume(-3f));
    }
