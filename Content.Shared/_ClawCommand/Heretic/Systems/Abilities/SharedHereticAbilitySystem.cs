...
public abstract partial class SharedHereticAbilitySystem : EntitySystem
{
        ...
        EntityUid? target)
    {
        ...
        // Upstream SharedGunSystem has no SetTarget for projectile homing; heretic
        // homing-projectile abilities lose homing but still fire in the requested direction.
        _ = target;
    ...
    /// <summary>
    ///     Heals (or damages) a target across multiple damage axes. Adapted from Goob's surgery-aware version
    ///     to use the fork's standard damage + stamina + bloodstream systems instead of wounds/pain/consciousness.
    /// </summary>
    /// <param name="uid">Entity to heal</param>
    /// <param name="toHeal">DamageSpecifier to apply (negative values heal); null = full heal of all damage</param>
    /// <param name="boneHeal">Heal blunt damage by this amount; negative = damage; null = full heal blunt</param>
    /// <param name="painHeal">Heal stamina damage by this amount; negative = damage; null = clear stamina damage</param>
    /// <param name="woundHeal">Heal slash + piercing damage by this amount; negative = damage; null = full heal of both</param>
    /// <param name="bloodHeal">Restore blood by this amount; null = fully restore</param>
    /// <param name="bleedHeal">Reduce bleeding by this amount; null = fully halt bleeding</param>
        ...
        FixedPoint2? bleedHeal)
    {
        ...
        if (toHeal != null)
        {
            _dmg.TryChangeDamage(uid, toHeal, true, false);
        }
        else
        {
        ...
        // Bone heal -> Blunt damage healing
        if (boneHeal != FixedPoint2.Zero)
        {
            var bluntAmount = boneHeal ?? FixedPoint2.New(-1000);
            ApplyDamageToType(uid, "Blunt", bluntAmount);
        }

        // Wound heal -> Slash + Piercing damage healing
        if (woundHeal != FixedPoint2.Zero)
        {
            var slashAmount = woundHeal ?? FixedPoint2.New(-1000);
            ApplyDamageToType(uid, "Slash", slashAmount);
            ApplyDamageToType(uid, "Piercing", slashAmount);
        }

        // Pain heal -> stamina damage healing (negative pain = restore stamina)
        if (painHeal != FixedPoint2.Zero && TryComp<StaminaComponent>(uid, out var stam))
        {
            if (painHeal == null)
                _stam.TakeStaminaDamage(uid, -stam.StaminaDamage, stam, visual: false);
            else
                _stam.TakeStaminaDamage(uid, painHeal.Value.Float(), stam, visual: false);
        }
        ...
        if (bloodHeal == FixedPoint2.Zero ||
            ...
            blood.BloodSolution!.Value.Comp.Solution.Volume >= blood.BloodReferenceSolution.MaxVolume)
            return;

        if (bloodHeal == null)
        {
            _blood.TryModifyBloodLevel((uid, blood),
                blood.BloodReferenceSolution.MaxVolume - blood.BloodSolution.Value.Comp.Solution.Volume);
        }
        else
        {
            _blood.TryModifyBloodLevel((uid, blood),
    ...
    private void ApplyDamageToType(Entity<DamageableComponent?> uid, string damageTypeId, FixedPoint2 amount)
    {
        if (!Resolve(uid, ref uid.Comp, false))
            return;

        var spec = new DamageSpecifier();
        spec.DamageDict[damageTypeId] = amount;
        _dmg.TryChangeDamage(uid, spec, true, false);
    }
}
